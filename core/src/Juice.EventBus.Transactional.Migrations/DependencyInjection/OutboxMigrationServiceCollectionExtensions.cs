                ...
                switch (provider)
                {
                    case "PostgreSQL":
                        ...
                        options.UseNpgsql(
                           configuration.GetConnectionString(connectionName),
                            x =>
                            {
                                x.MigrationsHistoryTable("__EFOutboxMigrationsHistory", schema);
                                x.MigrationsAssembly("Juice.EventBus.Transactional.Migrations.PostgreSQL");
                            });
                        break;
                    ...
                    case "SqlServer":
                        options.UseSqlServer(
                            ...
                            {
                                x.MigrationsHistoryTable("__EFOutboxMigrationsHistory", schema);
                                x.MigrationsAssembly("Juice.EventBus.Transactional.Migrations.SqlServer");
                            });
                        break;
                    default:
