...
namespace Juice.EventBus.Delivery.Processing
{
    ...
    internal sealed class DeliveryHostedService<TContext> : BackgroundService
    {
        ...
        private async Task ProcessBatchAsync(DeliveryPolicy policy, CancellationToken cancellationToken)
        {
            ...
            var intent = scope.ServiceProvider.GetRequiredKeyedService<IOutboxIntent<TContext>>(_intent);
            var publisher = scope.ServiceProvider.GetRequiredKeyedService<IEventPublisher>(_publisher);
